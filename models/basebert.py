...
class BertImageEmbeddings(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        img_embeddings = self.image_embeddings(input_ids).unsqueeze(1)
...
class BertLayer(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        self_output = self.self(input_tensor, attention_mask)
        attention_output = self.selfoutput(self_output)
        attention_output = self.LayerNorm(attention_output+input_tensor)
        intermediate_output = self.intermediate(attention_output)
        ffc_output = self.output(intermediate_output)
        layer_output = self.LayerNorm(ffc_output+attention_output)
        return layer_output
